...
func TestCpModel_Schedule_NoOverlap(t *testing.T) {
	...
	t.Logf("Optimal Schedule Length: %v\n", status.ObjectiveValue)
	t.Logf("Task 0 starts at %v\n", status.Solution[start0.Index()])
	t.Logf("Task 1 starts at %v\n", status.Solution[start1.Index()])
	t.Logf("Task 2 starts at %v\n", status.Solution[start2.Index()])
...
func Test_AllSolutions(t *testing.T) {
	...
	x := model.NewIntVar(0, int64(numVals-1), "x")
	y := model.NewIntVar(0, int64(numVals-1), "y")
	z := model.NewIntVar(0, int64(numVals-1), "z")

	model.AddAllDifferent([]IntVar{*x, *y}, "x and y different")

	sp := InitScalProd([]IntVar{*x, *y, *z}, []int{1, 2, 3})
	...
	solver := CpSolver{}
	_ = solver.SolveAllSolutions(*model, func(response CpSolverResponse) {
		solutionCount++
	})
